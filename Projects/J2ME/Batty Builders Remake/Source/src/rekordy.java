...
public class rekordy extends Canvas implements Runnable {
	...
	public void wczytajrekordy() {
		...
		try {
			recordStore = RecordStore.openRecordStore("battybwyniki", true);
	...
	public void dodajwynik(long pkt, String im) {
		...
		for (int a=0; a<=iloscmiejsc-1; a++)
			if ((r_wynik[a]==pkt) && (r_imie[a]==im))
			{
				...
				if (odmiejsca>iloscmiejsc-6) odmiejsca=iloscmiejsc-6;
				break;
			}
	...
	public void paint(Graphics g) {
		...
		g.drawString("TABLICA REKORDÓW", 50,1,Graphics.TOP|Graphics.HCENTER);
		...
		for (int a=0; a<=5; a++)
		{
			...
			g.drawString("" + r_wynik[a+odmiejsca], 100, 12+a*10,Graphics.TOP|Graphics.RIGHT);
		}
   ...
   protected void keyPressed(int keyCode)
   {
		...
		break;
		case KEY_NUM8:
			if (odmiejsca<iloscmiejsc-6) odmiejsca++;
			repaint();
		break;
		}
